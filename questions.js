...
const uniqueKey = 'G8_Addition And Subtraction With Rational Numbers'
...
function buildRationalSet(answers, question){
  ...
  question.question.forEach(function(part){
    ...
    if(part.fraction){
      ...
      var whole = part.fraction.whole === '______'
          ? parseInt(answers[index++])
          : parseInt(part.fraction.whole || 0)

      var num = part.fraction.numerator === '______'
          ? parseInt(answers[index++])
          : parseInt(part.fraction.numerator)

      var den = part.fraction.denominator === '______'
          ? parseInt(answers[index++])
          : parseInt(part.fraction.denominator)
...
// Converts flat answer array into human-readable fraction strings like "3/2, 2, 5/2"
  ...
  if (Array.isArray(question.question)) {
    ...
    question.question.forEach(function(part) {
      ...
      if (typeof part === 'string') {
        ...
        var wholeBlank = part.fraction.whole === '______'
        var numBlank = part.fraction.numerator === '______'
        var denBlank = part.fraction.denominator === '______'
...
function loadInputQuestion(randomQuestion, optionsElement, index) {
  ...
  if (whole === '______') {
     wholeSpan.innerHTML = '';
    let input = createInputElement(randomQuestion.input[inputIndex]);
    wholeSpan.appendChild(input);
    inputIndex++;
  } else {
  ...
  if (numerator === '______') {
    let input = createInputElement(randomQuestion.input[inputIndex]);
    numeratorSpan.appendChild(input);
    inputIndex++;
  } else {
    numeratorSpan.innerHTML = numerator;
  }

  // Denominator
  if (denominator === '______') {
    let input = createInputElement(randomQuestion.input[inputIndex]);
    denominatorSpan.appendChild(input);
    inputIndex++;
  } else {
    denominatorSpan.innerHTML = denominator;
  }
...
function loadOptionQuestion (randomQuestion, optionsElement, index) {
  ...
  randomQuestion.options.forEach(function (option, idx) {
    ...
    if (option.image) {
      ...
      optionImage.style.maxWidth = '400px'
      optionImage.style.height = '3px solid black'
      optionImage.style.border = "12px";
      optionImage.style.cursor = 'pointer'

      optionImage.style.border = "3px solid black"; // ← ADD THIS LINE before onclick

optionImage.onclick = function () {
    radioButton.checked = true

    // Reset all images to black border
    optionsElement.querySelectorAll("img").forEach(img => {
        img.style.border = "3px solid black";
    });

    // Highlight currently selected image with blue border
...
if (img) {
    // First reset all images to black, then highlight the selected one
    optionsElement.querySelectorAll("img").forEach(i => i.style.border = "3px solid black");
...
if (
  ...
  if (currentQuestion.unorderedFractions) {
    // Build sorted rational sets and compare
    var studentSet = buildRationalSet(answer, currentQuestion)
    var correctSet = buildRationalSet(currentQuestion.answer, currentQuestion)
  ...
  ansContent = buildDisplayString(correctAnswers, quesgroup)
} else {
  ansContent = buildDisplayString(correctAnswers, quesgroup)
}
...
if (givenAnswers && Array.isArray(givenAnswers)) {
  ...
  if (quesgroup.unorderedFractions) {
    ...
    isCorrect = JSON.stringify(studentSet) === JSON.stringify(correctSet)
    // Display as reconstructed fractions, not raw digits
    givenContent =
      "<span style='color: " +
      (isCorrect ? 'inherit' : 'red') +
      ";'>" +
      buildDisplayString(givenAnswers, quesgroup) +
      '</span>'
  } else {
    isCorrect = correctAnswers.every((correctAnswer, idx) => {
      return (
        givenAnswers[idx] &&
  ...
  givenContent = "<span style='color:red'>Not Answered</span>"
        ...
        if (ans.image) {
          ansContent = "<img src='" + ans.image + "' alt='Answer Image' style='width: 400px; height: 100px;'>"
  ...
  if (givenAnswer.image) {
    givenContent = "<img src='" +
      givenAnswer.image +
      "' alt='Given Answer Image' style='width: 400px; height: 100px;" +
    ...
    givenContent = "<span style='color: " +
      (isCorrect ? 'inherit' : 'red') +
      ";'>" +
      formatTextPartsForDisplay(givenAnswer.textParts) +
      '</span>'
  } else {
    givenContent = "<span style='color: " +
      (isCorrect ? 'inherit' : 'red') +
